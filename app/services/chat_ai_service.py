...
D. CORPORATE EVENTS ARE CUSTOM QUOTED: For any company, team, office, or corporate event, do NOT quote the $1,175, $1,350, or $1,400 prices from rules B and B2. Say corporate events are custom quoted because the price depends on location, group size, day of the week, and format, and that we are happy to call them back or send a custom written quote. Take their name, company, best phone or email, rough date, headcount, and location. We do not offer a half private, half public class: a group can buy a block of public tickets (it joins a public class alongside other guests, and large groups can get a group discount by calling (281) 408-4647 to discuss) or buy out a whole class privately.
...
def _pawty_calendar_block(months: int = 12) -> str:
    ...
    try:
        ...
        today = (dt.datetime.utcnow() - dt.timedelta(hours=6)).date()
    lines = [f"\nCALENDAR: Today is {today.strftime('%A, %B')} {today.day}, {today.year} (Houston time)."]
    week = [today + dt.timedelta(days=n) for n in range(14)]
    lines.append("Next 14 days: " + "; ".join(d.strftime("%a %b ") + str(d.day) for d in week) + ".")
    y, m = today.year, today.month
    for _ in range(months):
        days = calendar.monthrange(y, m)[1]
        sats = [d for d in range(1, days + 1) if dt.date(y, m, d).weekday() == 5]
        suns = [d for d in range(1, days + 1) if dt.date(y, m, d).weekday() == 6]
        first = calendar.day_name[dt.date(y, m, 1).weekday()]
        lines.append(f"{calendar.month_name[m]} {y}: the 1st is a {first}; Saturdays {', '.join(map(str, sats))}; Sundays {', '.join(map(str, suns))}.")
        m += 1
        if m == 13:
            m, y = 1, y + 1
    return "\n".join(lines) + "\n"
